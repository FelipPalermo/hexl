...
# /// ----- math methods ----- ///
    def __add__(self, value : Union[int, str]) -> "hexl" : 
        ...
        new_hexl = hex_functions.to_dec(str(self.ID)) + value
        return hexl.to_hex(new_hexl)

    def __sub__(self, value : Union[int, str]) -> "hexl" : 

        new_hexl = int(hex_functions.to_dec(str(self.ID))) - value
        return hexl.to_hex(new_hexl)

    def __mul__(self, value : int) -> "hexl" : 

        new_hexl = hex_functions.to_dec(self.ID) * value
        return hexl(hexl.to_hex(new_hexl))
